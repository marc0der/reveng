...
def load(path: pathlib.Path) -> pd.DataFrame:
    ...
    # Cost per 1k output tokens — the closest proxy for "value per pound".
    df["cost_per_1k_out"] = (
        df["cost_usd"] / (df["output_tokens"] / 1000).where(df["output_tokens"] > 0)
    ).fillna(0)
...
c4.metric("Mean cache hit", f"{view['cache_hit_pct'].mean():.0f}%")
c5.metric("Total time", f"{view['duration_s'].sum() / 3600:,.1f} h")
