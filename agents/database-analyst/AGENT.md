...
tools: Read, Write, Edit, Glob, Grep, Bash(mkdir*)
memory: project
---
...
- **Write the file incrementally.** An exhaustive analysis of a large schema cannot be emitted in a single response — the file ends up truncated partway through, with later sections silently missing. Write the metadata block and first section with Write, then append each subsequent section with Edit (using the file's current last lines as `old_string`). Split any single section that is itself very large into several appends. Do not create scratch or part files alongside the output; append directly to `output/database-analysis.md`.
