...
3. **Produce semantic HTML** following these rules strictly:
   ...
   - Preserve all visible text content exactly as shown in the screenshot, **except** for personally identifiable information (PII) — replace any real names, email addresses, phone numbers, national identifiers, addresses, or other identifying details with realistic fake equivalents (e.g. "John Smith" → "Jane Cooper", "john.smith@example.com" → "jane.cooper@example.com"). Keep the fake data consistent within a single file (i.e. the same real name always maps to the same fake name).
